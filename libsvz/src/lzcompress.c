...
uint8_t * read_file( const char * const name, long * const file_sizep )
{
	...
	if( !f )
	{
		fprintf( stderr, "lzcompress: Can't open input file '%s': %s\n",
			 name, strerror( errno ) );
		return 0;
	}
	...
	if( !buffer )
	{ fputs( "lzcompress: Not enough memory.\n", stderr ); return 0; }
	file_size = fread( buffer, 1, buffer_size, f );
	while( file_size >= buffer_size )
	{
		if( buffer_size >= LONG_MAX )
		{
			fprintf( stderr, "lzcompress: Input file '%s' is too large.\n", name );
	...
	if( ferror( f ) || !feof( f ) )
	{
		fprintf( stderr, "lzcompress: Error reading file '%s': %s\n",
			 name, strerror( errno ) );
		free( buffer ); return 0;
	}
	fclose( f );
...
*/
uint8_t * lzcompress( const uint8_t * const data, const long size, const int level, long * const out_sizep )
{
	struct Lzma_options
	{
...
/* Decompresses 'size' bytes from 'data'. Returns the address of a
   ...
   In case of error, returns 0 and does not modify '*out_sizep'.
*/
uint8_t * lzdecompress( const uint8_t * const data, const long size, long * const out_sizep )
{
